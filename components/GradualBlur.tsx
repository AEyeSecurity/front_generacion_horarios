...
type GradualBlurProps = PropsWithChildren<{
  ...
  target?: 'parent' | 'page';
  showWhen?: 'always' | 'not-at-start' | 'not-at-end';
...
const DEFAULT_CONFIG: Partial<GradualBlurProps> = {
  ...
  target: 'parent',
  showWhen: 'always',
  scrollTargetSelector: '[data-schedule-scroll]',
  className: '',
  style: {}
};
...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  useEffect(() => {
    if (config.showWhen === 'always') {
      setScrollVisible(true);
      return;
    }

    const resolveScrollTarget = (): Window | HTMLElement | null => {
      if (config.target === 'page') return window;
      const parent = containerRef.current?.parentElement;
      if (!parent) return null;
      return parent.querySelector(config.scrollTargetSelector) as HTMLElement | null;
    };
    ...
    const updateVisibility = () => {
      if (scrollTarget instanceof Window) {
        ...
        setScrollVisible(config.showWhen === 'not-at-start' ? top > 1 : top < maxTop - 1);
        return;
      }
  ...
  }, [config.showWhen, config.scrollTargetSelector, config.target]);
  ...
  const finalVisible = isVisible && scrollVisible;
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: finalVisible ? 'translate3d(0,0,0)' : hiddenTransform,
        transition: `opacity ${config.duration} ${config.easing}, transform ${config.duration} cubic-bezier(0.22,1,0.36,1)`
      }}
