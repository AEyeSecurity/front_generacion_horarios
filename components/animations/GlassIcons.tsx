...
const gradientMapping: Record<string, string> = {
  ...
  gray: "linear-gradient(hsl(210, 10%, 45%), hsl(210, 10%, 35%))",
};
...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  ...
  return (
    <div className={`grid gap-[5em] grid-cols-2 md:grid-cols-2 mx-auto py-[3em] overflow-visible ${className || ""}`}>
      {items.map((item, index) => {
        ...
        if (item.href) {
          return (
            <Link key={index} href={item.href} className="justify-self-center">
              {content}
            </Link>
          );
        }
