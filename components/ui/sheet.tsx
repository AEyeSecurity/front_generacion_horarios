...
function SheetContent({
  ...
  /** Optional top offset in px (number) or CSS length (string) to render below a fixed header */
  offsetTop?: number | string
}) {
  const { t } = useI18n()
  const top =
    typeof offsetTop === "number" ? `${offsetTop}px` : offsetTop || undefined
  const contentBase =
  ...
  return (
    <SheetPortal>
      {/* If offsetTop is provided, push overlay down */}
      <SheetOverlay style={top ? { top, right: 0, left: 0, bottom: 0 } : undefined} />
