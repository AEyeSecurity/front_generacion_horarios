// Proxy for /api/invitations -> BACKEND_URL/api/invitations
...
export async function GET(req: Request) {
  ...
  let r = await fetch(`${process.env.BACKEND_URL}/api/invitations/${qs ? `?${qs}` : ""}`, { cache: "no-store" });
  ...
  r = await fetch(`${process.env.BACKEND_URL}/api/invitations/${qs ? `?${qs}` : ""}`, {
...
export async function POST(req: Request) {
  ...
  let r = await fetch(`${process.env.BACKEND_URL}/api/invitations/`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body,
    cache: "no-store",
  ...
  const { tokens, refresh } = refreshed;
  r = await fetch(`${process.env.BACKEND_URL}/api/invitations/`, {
    method: "POST",
