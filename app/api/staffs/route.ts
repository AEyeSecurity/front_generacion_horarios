...
// GET /api/staffs?grid=<ID>
export async function GET(req: Request) {
  const url = new URL(req.url);
  const qs = url.searchParams.toString();
  let r = await fetch(`${process.env.BACKEND_URL}/api/staffs/${qs ? `?${qs}` : ""}`, { cache: "no-store" });
  ...
  r = await fetch(`${process.env.BACKEND_URL}/api/staffs/${qs ? `?${qs}` : ""}`, {
...
export async function POST(req: Request) {
  ...
  let r = await fetch(`${process.env.BACKEND_URL}/api/staffs/`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body,
    cache: "no-store",
  ...
  const { tokens, refresh } = refreshed;
  r = await fetch(`${process.env.BACKEND_URL}/api/staffs/`, {
    method: "POST",
