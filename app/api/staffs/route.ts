...
export async function GET(req: Request) {
  ...
  let r = await fetch(`${getApiBaseUrl()}/api/staffs/${qs ? `?${qs}` : ""}`, { cache: "no-store" });
  ...
  r = await fetch(`${getApiBaseUrl()}/api/staffs/${qs ? `?${qs}` : ""}`, {
...
export async function POST(req: Request) {
  ...
  let r = await fetch(`${getApiBaseUrl()}/api/staffs/`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body,
    cache: "no-store",
  ...
  const { tokens, refresh } = refreshed;
  r = await fetch(`${getApiBaseUrl()}/api/staffs/`, {
    method: "POST",
