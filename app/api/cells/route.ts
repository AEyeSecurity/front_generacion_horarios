...
// GET /api/cells?grid=<ID>
export async function GET(req: Request) {
  const url = new URL(req.url);
  const qs = url.searchParams.toString();
  let r = await fetch(`${process.env.BACKEND_URL}/api/cells/${qs ? `?${qs}` : ""}`, { cache: "no-store" });
  ...
  r = await fetch(`${process.env.BACKEND_URL}/api/cells/${qs ? `?${qs}` : ""}`, {
...
// POST /api/cells -> BACKEND /api/cells/
export async function POST(req: Request) {
  const body = await req.text();
  let r = await fetch(`${process.env.BACKEND_URL}/api/cells/`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body,
    cache: "no-store",
  ...
  const { tokens, refresh } = refreshed;
  r = await fetch(`${process.env.BACKEND_URL}/api/cells/`, {
    method: "POST",
